...
func (ini *IniConfig) parseData(dir string, data []byte) (*IniConfigContainer, error) {
	...
	for {
		...
		//It might be a good idea to throw a error on all unknown errors?
...
// or under gin in default temporary directory(/tmp/gin[-username]).
func (ini *IniConfig) ParseData(data []byte) (Configure, error) {
	dir := "gin"
	currentUser, err := user.Current()
	if err == nil {
		dir = "gin-" + currentUser.Username
	}
